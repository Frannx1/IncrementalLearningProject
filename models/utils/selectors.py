...
class Selector:
    ...
    def get_nearest_centers(self, centers, features):
        ...
        """
        centers, norm_features = self.prepare(centers, features)
    ...
    def get_nearest_feature(self, center, features):
        ...
        center, norm_features = self.prepare(center, features)
        closest_idx = self.calculate_distances(center, norm_features).argmin().item()
        return closest_idx

    def prepare(self, centers, features):
        ...
        # norm_features = self.metric.normalize(features, dim=1).unsqueeze(2) # (batch_size, feature_size, 1)
        norm_features = features.unsqueeze(2)
